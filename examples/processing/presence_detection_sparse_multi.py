...
def main():
    ...
    for i in range(num_of_sensors):
        pg_updaters.append(PGUpdater(sensor_config, processing_config, session_info, sensor_num=i+1))
    ...
    processors = []
    for i in range(num_of_sensors):
        processors.append(PresenceDetectionSparseProcessor(sensor_config, processing_config, session_info))
...
class PGUpdater:
    def __init__(self, sensor_config, processing_config, session_info, sensor_num = False):
    ...
    def setup(self, win):
        if self.sensor_num == False:
            win.setWindowTitle("Acconeer presence detection example")
        else:
            win.setWindowTitle("SENSOR: {} | Acconeer presence detection example".format(self.sensor_num))
        ...
        if self.sensor_num == False:
            ...
            self.data_plot = win.addPlot(
                    row=0,
                    col=0,
                    title="SENSOR: {} | Frame (blue), fast (orange), and slow (green)".format(self.sensor_num),
                    )
